...
## ✨ 0.3.13 업데이트
- **GPIO 제어 개선**: 전원(3.3V, 5V) 및 그라운드 핀의 잘못된 설정을 방지하도록 가드 로직을 추가했습니다.
- **UI 시각화**: 설정되지 않은 GPIO 핀을 회색으로 표시하여 시각적 구분을 강화했습니다.
- **사용성 개선**: 핀 상세 보기에서 '전체 보기' 버튼을 추가하여 다시 목록으로 돌아가기 편리하게 개선했습니다.
- **버전 동기화**: 엔진과 웹 관리자의 버전을 0.3.13으로 업데이트했습니다.
