...
> [!TIP]
> **macOS 사용자 참고**: Apple에 등록된 유료 개발자 인증서로 서명되지 않았으므로 처음 실행 시 '악성 코드가 없음을 확인할 수 없습니다'라는 경고가 뜰 수 있습니다. **'시스템 설정 > 개인정보 보호 및 보안'**에서 **'확인 없이 열기'**를 클릭하거나, 앱 아이콘에서 **'마우스 오른쪽 버튼 클릭 > 열기'**를 선택하여 실행해 주세요.
