...
## ✨ 0.4.3 업데이트
- **업그레이드 및 자동 재시작**: `marubot upgrade` 실행 시 업그레이드 후 시스템이 자동으로 재시작되도록 개선했습니다.
- **AI 버전 제어 및 업데이트**: AI 에이전트가 직접 버전을 확인하고 사용자의 요청에 따라 업그레이드를 수행할 수 있는 도구를 추가했습니다.
- **Shadcn UI 다이얼로그**: Web Admin의 모든 브라우저 기본 대화상자를 세련된 Shadcn UI 소형 다이얼로그로 교체하여 일관된 UX를 제공합니다.
- **버전 관리 중앙화**: 엔진 내부의 버전 관리 로직을 통합하여 업데이트 체크 및 관리의 정확도를 높였습니다.
