...
## ✨ 0.4.2 업데이트
- **GPIO 설정 영구 저장**: 기존에 핀 번호만 저장되던 방식에서 **입출력 모드(IN, OUT 등)**가 함께 저장되도록 개선하여 새로고침 후에도 설정이 유지됩니다.
- **핀 선택 UI 개선**: 숫자를 직접 입력하는 대신, 설정 가능한 **GPIO 핀 목록에서 선택(Select box)**할 수 있도록 변경하여 중복 설정을 방지하고 편의성을 높였습니다.
- **관리자 암호 보호**: `marubot onboard` 및 설치 시, 빈 값을 입력하면 기존 설정된 암호를 유지하도록 로직을 강화했습니다.
- **AI 연동 강화**: AI가 사용자가 설정한 핀 레이블을 인식하여 정확하게 제어할 수 있도록 도구 로직을 업데이트했습니다.
