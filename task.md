...
- [x] 텔레그램 및 채널 응답 이슈 디버깅 및 해결
    ...
    - [x] 텔레그램 마크다운 코드 블록 및 인라인 코드 변환 시 고유 인덱스 미부여로 인한 치환 오류 수정 (`telegram.go`)
...
- [x] **Task 22.2: 3단계 지능형 기억 아키텍처 (STM/LTM/Facts) 구현**
    ...
    - [x] **SSH 고도화:** 접속 전 로컬 키 사전 점검 및 자동 생성(Step 0) 로직 구현
- [x] **Task 22.3: 대화 맥락 복기 성능 검증 및 배포 패치 (v0.4.33)**
    - [x] `reloadCmd` 보강: `daemon-reload` 추가를 통한 업그레이드 즉시 반영 해결
    - [x] FTS5 핫픽스: 특수문자(`~`) 검색 에러 방지 새니타이징 로직 적용
    - [x] 업그레이드 수동 동기화: `upgrade` 시 `IDENTITY.md` 즉시 갱신 추가
...
- **2026-03-11**: MaruBot v0.4.35. **[SSH 및 자아 인식 패키지]** SSH 키 사전 생성 로직 추가 및 RAG 중복 Facts 정밀 세척 패치 완료.
- **2026-03-11**: MaruBot v0.4.33. **[RAG 및 가독성 핫픽스]** FTS5 특수문자 에러(`~`) 해결, 업그레이드 시 `IDENTITY.md` 즉각 동기화, 및 텔레그램 내 HTML 태그 출력 억제 패치 완료.
...
- **2026-04-18**: MaruBot v0.6.7 패치. 텔레그램 채널의 마크다운 코드 블록 변환 버그(모든 블록이 IP 주소로 치환되는 현상) 해결.
