...
## 2026-04-21
### 0.7.3
- **라즈베리 파이 빌드 안정화**: Go 1.25+ 미래 버전 종속성 유입 방지를 위한 `replace` 지시어 도입 및 패키지 버전(cdproto 등) 2024년 안정 버전으로 강제 고정.
- **설치 스크립트 고도화**: `install.sh`의 아카이브 다운로드 주소를 캐시 방지용 태그 주소로 변경하여 배포 신뢰성 확보.
- **정역학적 버전업 SOP 준수**: `versionup.md` 정석 절차에 따른 전방위 버전 동기화 (Identity, Docs, UI).

### 0.7.2
- **대화 히스토리 서버 저장**: 대화 자동 저장 및 사이드바 내역 관리(개별 삭제 포함) 기능 추가.
- **상세 시스템 모니터링**: Windows 환경 상세 하드웨어 메트릭(gopsutil) 연동.
- **UI/UX 고도화**: 대시보드 상세 다이얼로그 및 Progress UI 적용.
- **안정성 개선**: 의존성 정리 및 윈도우 빌드 보정.
