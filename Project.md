...
### 0.4.95
- 슬랙(Slack) 스레드(Thread) 응답 지원: 멘션(@bot) 시에는 스레드 답장으로, 일반 메시지에는 채널 직접 답장으로 지능형 분기 처리
- 아웃바운드 메시지 메타데이터 지원 강화
- 슬랙(Slack) 연동 강화: `app_mention` 이벤트 처리 추가 (채널 내 봇 멘션 시 응답 지원)
- 아웃바운드 메시지 전송 안정화: 메시지 전송 루프에 패닉 복구 로직 추가
- 버전 관리 정합성 최종 수정: 리눅스 업그레이드 전용 원격 저장소 강제 동기화
- 안정성 및 버전 업데이트
...
### 0.4.98
- 설정 자동 동기화 로직 추가: 프로그램 시작 시 `config.json` 파일을 최신 구조(슬랙 등 누락 필드 포함)로 자동 갱신 및 동기화하도록 개선
- 설정 파일 정형화: `feishu`, `maixcam` 등 지원하지 않는 구버전 필드 자동 제거 및 정리
- 안정화 및 버전 업데이트
