...
## ✨ 0.4.0 アップデート
<<<<<<< HEAD
- **Webhookチャネルの追加**: 外部サービスからHTTPリクエストを通じてMaruBotとリアルタイムに対話が可能になりました（同期応答をサポート）。
- **AI自己進化 (Self-Evolution)**: AIエージェントが自ら新しいスキルをインストールしたりツールを作成したりした後、自分自身を再起動（`reload`）して機能を拡張できるようになりました。
- **システム制御ツール (`system_control`)**: AIが直接MaruBotの状態を確認し、スキルのインストールやプロセスの再起動を実行できる能力を備えました。
=======
---

## ✨ 0.4.1 アップデート
- **GPIO制御の改善**: 電源(3.3V, 5V)およびグランドピンの誤設定を防止するガードロジックを追加しました。
- **UIの可視化**: 設定されていないGPIOピンを灰色で表示し、視覚的な区別を強化しました。
- **Webhookチャネルの追加**: 外部サービスからHTTPリクエストを通じて、MaruBotとリアルタイム対話が可能になりました。
- **AI自己進化 (Self-Evolution)**: エージェントが自律的にスキルをインストールしたり、新しいツールを作成し、リロードして機能を拡張できるようになりました。
- **操作性の向上**: ピン詳細ビューに「すべて表示」ボタンを追加し、一覧に戻りやすく改善しました。
- **バージョン同期**: エンジンとWeb Adminのバージョンを0.4.1に同期しました。
